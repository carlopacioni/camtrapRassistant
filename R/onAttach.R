.onAttach <- function(...) {
  ...
  local_version = utils::packageDescription('camtrapRassistant')

  # Grab the package information from CRAN
  #cran_version = .pkgVersionCRAN("camtrapRassistant")     # function defined in "variousOtherHelperFunctions.R"
  cran_version <- NULL
  ...
  if(exists("d"))  packageStartupMessage('This is camtrapRassistant version: ', local_version$Version, ' (', d,')')  # built on ', local_version$Date)
  if(exists("d2")) packageStartupMessage('This is camtrapRassistant version: ', local_version$Version, '    (', d2,')')  # built on ', local_version$Date)


  if(latest_version == 1) {
    packageStartupMessage('\n NEW VERSION AVAILABLE ONLINE: ', cran_version , ' !')
    packageStartupMessage('Download it from CRAN via:    install.packages("camtrapRassistant")\n')
  }
}
